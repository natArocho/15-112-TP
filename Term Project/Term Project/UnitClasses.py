...
class Unit(object):
    ...
    def battle(self, enemy, grid):
        ...
        hitBonus, powBonus = self.weaponTriangle(enemy)

        hitChance = self.accuracy + pAvoBuff + hitBonus - enemy.avoid - eAvoBuff 
        critRate = self.crit - enemy.stats["Luck"]
        if hitChance > 100: hitChance = 100
        battleRoll = random.randint(1,100)
        if battleRoll <= hitChance:
            damage = self.power + pDefBuf + powBonus - enemy.stats["Defense"] - eDefBuf
    ...
    def weaponTriangle(self, enemy):
        hitBonus = 0
        powBonus = 0
        if isinstance(self.equipped, Sword) and isinstance(enemy.equipped, Axe) \
         or isinstance(self.equipped, Lance) and isinstance(enemy.equipped, Sword) \
         or isinstance(self.equipped, Axe) and isinstance(enemy.equipped, Lance):
            hitBonus = 10
            powBonus = 2

        elif isinstance(self.equipped, Axe) and isinstance(enemy.equipped, Sword) \
         or isinstance(self.equipped, Sword) and isinstance(enemy.equipped, Lance) \
         or isinstance(self.equipped, Lance) and isinstance(enemy.equipped, Axe):
            hitBonus = -10
            powBonus = -2

        return hitBonus, powBonus
    ...
    def options(self, newX, newY, grid):
        ...
        dirs = [(0, 1), (1, 0), (0,-1), (-1,0)]
        for dRow, dCol in dirs:
            if newX+dRow in range(len(grid)) and newY+dCol in range(len(grid[0])) and\
             grid[newX+dRow][newY+dCol].unit != None \
    ...
    def drawOptions(self, screen, xDist, yDist, row, col):
        ...
        for i in range(len(self.optionList)):
            Unit.drawFont(screen, str(i+1)+": "+self.optionList[i], (xDist*col, yDist*(row+i)))

    @staticmethod
    def drawFont(screen, text, location):
        pygame.font.init()

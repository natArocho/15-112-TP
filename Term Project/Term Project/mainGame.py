...
class PygameGame(object):
    ...
    def __init__(self, width=800, height=800, fps=60, title="Fire Emblem: Carnegie 112"):
        self.width = width
        self.height = height
        self.fps = fps
        self.title = title
        ...
        self.legalTile = pygame.Surface((self.height//self.rows, self.width//self.cols))
        self.legalTile.set_alpha(112)
        self.legalTile.fill((0,0,255))

        self.attackTile = pygame.Surface((self.height//self.rows, self.width//self.cols))
        self.attackTile.set_alpha(112)
        self.attackTile.fill((255,0,0))
        ...
        #all unit sprites from https://www.spriters-resource.com/3ds/fireemblemawakening/
    ...
    def run(self):
        ...
        while playing:
            ...
            if len(Unit.teams["Player"]) == 0:
                Unit.drawFont(screen, "Your army has been defeated. Game Over.", (self.width//2, self.height//2))
                self.gameOver = True
            if len(Unit.teams["Enemy"]) == 0:
                score = (self.minutes*60 + self.seconds) // len(Unit.teams["Player"])
                Unit.drawFont(screen, "Congrats, you win! Score: "+str(score), (self.width//2, self.height//2))
                self.gameOver = True
            pygame.display.flip()
        pygame.quit()
